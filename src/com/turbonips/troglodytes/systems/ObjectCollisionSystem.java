...
public class ObjectCollisionSystem extends BaseEntitySystem {
	...
	protected void initialize() {
		...
		renderableMapper = new ComponentMapper<Renderable>(Renderable.class,
				world);
	}
	...
	protected void processEntities(ImmutableBag<Entity> entities) {
		...
		if (!layers.isEmpty()) {
			Resource resource = renderableMapper.get(layers.get(0))
					.getResource();
			if (resource != null) {
				for (int p = 0; p < players.size(); p++) {
					...
					if (renderableMapper.get(player) != null) {
						Resource playerResource = renderableMapper.get(player)
								.getResource();
						Image sprite = null;
						...
						if (playerResource != null) {
							...
							case CREATURE_ANIMATION:
								sprite = ((CreatureAnimation) playerResource
							...
							case IMAGE:
								...
								break;
							default:
								logger.error("player resource type is "
										+ playerResource.getType());
								break;
							}
						}

						ObjectType objectType = getObjectType(playerPosition,
								resource.getId(), sprite);
						if (objectType != null) {
							switch (objectType.getType()) {
							case ObjectType.WARP_OBJECT:
								WarpObject warpObject = (WarpObject) objectType;
								logger.info("Adding warp to player "
										+ warpObject.toString());
	...
	private ObjectType getObjectType(Position position, String mapId,
			Image sprite) {
		int topLeftY;
		int bottomLeftY;
		int topLeftX;
